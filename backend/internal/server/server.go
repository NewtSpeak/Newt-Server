...
import (
	...
	"github.com/newtspeak/newt-server/backend/internal/activityapi"
	"github.com/newtspeak/newt-server/backend/internal/adminpresence"
	...
	"github.com/newtspeak/newt-server/backend/internal/botapi"
	...
	"github.com/newtspeak/newt-server/backend/internal/customization"
	...
	"github.com/newtspeak/newt-server/backend/internal/oauth"
	...
	"github.com/newtspeak/newt-server/backend/internal/sfubridge"
	...
	"github.com/newtspeak/newt-server/backend/internal/web"
...
func New(cfg config.Config, db *gorm.DB, bus *eventbus.Bus, sfu ...httpapi.SFUOptions) (*gin.Engine, error) {
	...
	// 服务器时间（Newt-Desktop docs 08 §8-9：客户端显示 Restriction 剩余时长时
	...
	// 取代 internal/sfunode 的 WSS 控制通道实现（与真实 Newt-SFU 协议不兼容，已停止监听）。
